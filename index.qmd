...
The algorithm maintains a min-heap of $(f, \mathbf{s})$ pairs ordered by the $f$-score
...
::: {.content-text}
Rather than planning one long path from start to finish, we decompose the route into **per-segment problems**. For $K$ gates with entry and exit pairs $(\mathbf{w}_{\text{entry}}^k, \mathbf{w}_{\text{exit}}^k)$, the segments are
