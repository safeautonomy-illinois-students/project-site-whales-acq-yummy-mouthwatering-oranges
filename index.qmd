...
$\mathbf{w}_{\text{entry}}$ is the point the drone must reach before the gate, and $\mathbf{w}_{\text{exit}}$ is the point it must clear on the far side. Three additional high-priority waypoints—the entry approach, the gate center $\mathbf{p}_g$, and the exit approach—are always inserted explicitly into the final waypoint list (see the Waypoint Assembly section below), guaranteeing the drone passes through the center of every gate regardless of what the grid planner chooses.
:::
...
::: {.content-text}
The search operates on grid indices rather than metric coordinates. Each cell has up to 26 neighbors (the full 3D Moore neighborhood), reached by steps $(\delta x, \delta y, \delta z) \in \{-1,0,1\}^3 \setminus \{(0,0,0)\}$. The movement cost of each step equals its Euclidean length:
...
The final output is a pair of arrays: an $(N \times 3)$ array of waypoint coordinates and an $(N,)$ array of weights. These are handed directly to the spline-smoothing stage described in the next section.
:::
